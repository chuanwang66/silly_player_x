...
{
	//os_process_pipe_destroy(): close pipe handle -> callee process fread() returns 0 -> callee process exits
	int ret = 0;
...
int os_process_process_destroy(os_process_pipe_t *pp)
{
	//os_process_process_destroy(): terminate callee process directly
	int ret = 0;
	...
	if (pp) {
		...
		ret = TerminateProcess(pp->process, 0) != 0 ? 0 : -1;	//TerminateProcess() is asynchronous
		WaitForSingleObject(pp->process, INFINITE);
		//if (GetExitCodeProcess(pp->process, &code))
		//	ret = (int)code;

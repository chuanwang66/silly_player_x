...
void *os_dlopen(const char *path)
{
	...
	blog(LOG_DEBUG, "os_dlopen path:%s", dll_name.array);
...
int os_get_config_path(char *dst, size_t size, const char *name)
{
	...
	*/
	...
	char *cur_gbk = ".";
	os_mbs_to_utf8_ptr(cur_gbk, strlen(cur_gbk), &ptr); //gbk->utf8
	...
	if (path.len + 1 > size) {
		//path.len + 1: path with terminated null
		//size: destination len
		//destination is TOO SHORT!
		return -1;
	}
	else {
		strncpy(dst, path.array, path.len);
		*(dst + path.len) = 0;
		return (int)strlen(dst);
	}
}

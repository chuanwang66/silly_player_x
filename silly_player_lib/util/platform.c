...
static void decipher(void *buffer, long offset, size_t count) {
	unsigned char *ptr = NULL;
	long i = 0;
	unsigned char keys[16] = {57, 56, 55, 54, 53, 52, 51, 50, 49, 48, 76, 73, 78, 87, 69, 73 };
	
	for (; i < count; ++i) {
		ptr = (unsigned char *)buffer + offset + i;
		unsigned char e = (*ptr) ^ keys[i & 15];
		*ptr = e;
	}
}

size_t os_fread_utf8_s(FILE *file, char **pstr)
{
	size_t size = 0;
	size_t size_read;
	size_t len = 0;
...
char *os_quick_read_utf8_file_s(const char *path)
{
	FILE *f = os_fopen(path, "rb");
	char *file_string = NULL;
